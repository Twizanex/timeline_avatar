<?php
   # ========================================================================#
   #  The following functions are taken from class imageLib () [available from
   #  https://github.com/Oberto/php-image-magician ], and are used with written permission
   #  from Jarrod Oberto.
   #   
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
  ...
  #       forceStretch can be applied to options 1,2,3 and 4
    ...
    if (!$this->image) { if ($this->debug) { die('file ' . $this->getFileName() .'During image resize, we found the file was missing or invalid'); }else{ die(); }};
    ...
    imagecopyresampled($this->imageResized, $this->image, 0, 0, 0, 0, $optimalWidth, $optimalHeight, $this->width, $this->height);
     ...
     // *** TM: 04/24/2014 Added two lines bellow Set new "original" sizes to correct padding calculations By:
     // **  https://github.com/astraliens/php-image-magician/commit/d5796a127fa2a85a7db6f0636d7e218a071d316c

     $this->width=$optimalWidth; // TM
     $this->height=$optimalHeight; // TM
...
## --------------------------------------------------------
    ...
    #
  {
    ...
    if (!$this->image) { if ($this->debug) { die('file ' . $this->getFileName() .'During image croping, we found the file was missing or invalid'); }else{ die(); }};
